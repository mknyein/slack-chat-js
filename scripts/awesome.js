...
test.prototype.loadMessages = function() {
    ...
    // Reloads the last 12 messages after delete msg.
    var setdeleteMessage = function(data) {
        var val = data.val();
        this.deleteMessage(data.key);
    }.bind(this);
    this.messagesRef.limitToLast(12).on('child_removed', setdeleteMessage);
};//loadMessages function end

// Saves a new message on the Firebase DB.
test.prototype.saveMessage = function(e) {
    e.preventDefault();
    ...
    if (this.messageInput.value && this.checkSignedInWithMessage()) {
        ...
        this.messagesRef.push({
            ...
            test.resetMaterialTextfield(this.messageInput);
            this.toggleButton();
        }.bind(this)).catch(function(error) {
            console.error('Error writing new message to Firebase Database', error);
        });
    }
};//saveMessage function end

test.prototype.setImageUrl = function(imageUri, imgElement) {
...
test.prototype.saveImageMessage = function(event) {
...
};//onAuthStateChanged function end

// Returns true if user is signed-in. Otherwise false and displays a message.
test.prototype.checkSignedInWithMessage = function() {
    ...
    this.signInSnackbar.MaterialSnackbar.showSnackbar(data);
    return false;
};//checkSignedInWithMessage function end

// Resets the given MaterialTextField.
test.resetMaterialTextfield = function(element) {
    element.value = '';
...
'<div class="name"></div>' +
'<button type="input" class="delbtn" onclick="removemsg(this.id)">remove</button>' +
'</div>';
...
function removemsg(data) {
  // body...
  firebase.database().ref('awesome-chat').child(data).remove();
...
test.prototype.displayMessage = function(key, name, text, picUrl, imageUri) {
    ...
    div.querySelector('.delbtn').id = key;
...
// fields.
test.prototype.toggleButton = function() {
...
};//checkSetup function end

window.onload = function() {
    window.chat = new test();
};


// Displays a Message in the UI.
test.prototype.deleteMessage = function(key) {
    var div = document.getElementById(key);
    // If an element for that message does not exists yet we create it.
    if (div) {
      div.remove();
